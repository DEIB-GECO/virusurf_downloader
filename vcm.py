...
def create_or_get_experiment(sample: VirusSample):
...
def create_or_get_sequencing_project(sample: VirusSample):
...
def create_or_get_host_sample(sample: VirusSample):
    tree = sample.underlying_xml_element_tree()
...
def create_sequence(virus_sample, virus_id: Virus, experiment: ExperimentType, host_sample: HostSample, sequencing_project: SequencingProject):
    sequence = Sequence(accession_id=virus_sample.primary_accession_number(),
...
def create_or_get_annotation(sample: VirusSample, sequence: Sequence):
    tree = sample.underlying_xml_element_tree()
...
def get_virus_sample_accession_ids(virus_specie_taxon_id: int) -> (int, List[int]):
    ...
    # get non-refseq accession ids (increase return_max_records if not enough)
    non_refseq_accessions_ids = list()
    RECORDS_PER_PAGE = 1000
    page_number = 0
    total_records = 1  # just a positive number to start the loop (the real value is then taken from the response)
