...
def run(db_user, db_password, db_port):
    ...
    if inp == 'genbank_gisaid':
        ...
        run(db_user, db_password, db_port)
        ...
        genbank_gisaid.run(db_user, db_password, db_port)
        from overlaps.genbank_nmdc import genbank_nmdc
        genbank_nmdc.run(db_user, db_password, db_port)
        from overlaps.gisaid_nmdc import gisaid_nmdc
        gisaid_nmdc.run(db_user, db_password, db_port)
        from overlaps.coguk_nmdc import coguk_nmdc
        coguk_nmdc.run(db_user, db_password, db_port)
        from overlaps.coguk_gisaid import coguk_gisaid
        coguk_gisaid.run(db_user, db_password, db_port)
        from overlaps.coguk_genbank import coguk_genbank
        coguk_genbank.run(db_user, db_password, db_port)
    else:
