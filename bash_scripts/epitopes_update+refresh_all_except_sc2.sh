...
# Delete old epitopes
echo "* Deletion of old non-SC2 epitopes..." | tee -a $log_file_path
cd $virusurf_dir
psql -h localhost -U geco -d ${database_name} -c "
...
echo "* Refresh ALL-but-SC2 eptiope variants and info materialized view"
psql -h localhost -U geco -d ${database_name} -c "
-- bombali ebolavirus
...
echo "* Update of ALL-but-SC2 epitopes completed on database ${database_name}." | tee -a $log_file_path