...
sleep 10
# internal variables
log_file_path=${virusurf_dir}logs/daily_update_genbank.log
...
delayed_stop_genbank() {
  cd $virusurf_dir
  sleep 12h
  echo "* Stopping import of genbank sars cov 2 on ${database_name}" | tee -a $log_file_path
...
# # Stop update of COGUK that may have been running until now because of an old execution of this script
cd $virusurf_dir
echo "Stopping import of coguk sars cov 2 on ${database_name}" | tee -a $log_file_path
python main.py stop import ${database_name} coguk


# # Update DB with new sequences
delayed_stop_genbank &    # stop genbank's pipeline in X hours
cd $virusurf_dir
echo "* Begin update of NCBI SC2 at $(timestamp)" | tee -a $log_file_path
python main.py import ${database_name} sars_cov_2 0 30000
check_exit_code "$?"
...
python main.py import ${database_name} coguk 0 9000
check_exit_code "$?"
